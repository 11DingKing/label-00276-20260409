...
function ProductionPage() {
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 80,
      render: (priority) => {
        const config = priorityConfig[priority] || { text: '中', color: 'blue' }
  ...
  return (
    <div>
      ...
      <Modal
        ...
        getContainer={false}
      >
        <Form form={form} layout="vertical" initialValues={{ priority: 3, quantity: 1 }}>
          <Row gutter={16}>
            <Col span={12}>
              <Form.Item
                name="product_id"
      ...
      >
        {currentOrder && (
          <>
            <Descriptions bordered column={2} size="small" style={{ marginBottom: 24, background: '#fafafa', borderRadius: 8 }}>
              ...
              <Descriptions.Item label="状态">
                <Tag color={statusConfig[currentOrder.status]?.color}>
                  {statusConfig[currentOrder.status]?.text || currentOrder.status}
                </Tag>
              </Descriptions.Item>
            ...
            <Title level={5} style={{ marginBottom: 20 }}>生产流程</Title>
            <div style={{ 
              ...
              <Steps
                ...
                items={processes.map((proc, index) => ({
                  title: (
                    <span style={{ 
                      fontWeight: 600, 
                      color: proc.status === 'COMPLETED' ? '#52c41a' : proc.status === 'IN_PROGRESS' ? '#1890ff' : '#666'
                    }}>
                      {proc.process_name}
                    </span>
                  ),
                  ...
                  status: proc.status === 'COMPLETED' ? 'finish' : proc.status === 'IN_PROGRESS' ? 'process' : 'wait',
                }))}
              />
            </div>
          </>
